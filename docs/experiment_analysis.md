...
| Exp5b | ASE | diverse (walk+spinkick+dance) | 3.06h | 500M | 3814 |
...
> **Bug 修复说明**: 训练期间记录的 AMP/ASE tracking error 数据存在 bug（`amp_env.py:183-186` 已修复）。
> 修复不影响训练过程（奖励/策略/判别器均不受影响）。
>
> **重评估已完成**: 使用 `eval_tracking_error.py` 对全部 9 个实验重新评估，
> 配置：3 seeds (42, 123, 456) × 4096 episodes/seed，统一 `pose_termination=False`。
> 下文中所有 tracking error 数据均来自重评估结果（mean ± std）。
>
> **坐标系说明**: DM 奖励在世界坐标系中跟踪全局位置（`track_root=True`），AMP 不跟踪。
> 因此 `root_pos_err` 对 DM 有天然优势。**`body_pos_err` 和 `body_rot_err`（root-relative）是更公平的对比维度。**
...
#### 追踪误差对比（重评估数据，3 seeds × 4096 episodes，pose_termination=False）

| 指标 | Exp1 (DM walk) | Exp3 (AMP walk) | DM 更优倍数 |
|------|-----------------|------------------|-------------|
| Root_Pos_Err (m) | **0.0238 ± 0.0003** | 0.4497 ± 0.0008 | 18.9× ★ |
| Root_Rot_Err (rad) | **0.0259 ± 0.0006** | 0.0796 ± 0.0006 | 3.1× |
| Body_Pos_Err (m) | **0.0085 ± 0.0001** | 0.0789 ± 0.0002 | **9.3×** |
| Body_Rot_Err (rad) | **0.0441 ± 0.0005** | 0.2871 ± 0.0007 | **6.5×** |
| Dof_Vel_Err (rad/s) | **0.2886 ± 0.0005** | 0.9951 ± 0.0005 | **3.4×** |
| Root_Vel_Err (m/s) | **0.0471 ± 0.0003** | 0.1964 ± 0.0004 | 4.2× ★ |
| Root_Ang_Vel_Err (rad/s) | **0.0865 ± 0.0014** | 0.3878 ± 0.0016 | 4.5× |

> ★ Root_Pos_Err 和 Root_Vel_Err 对 DM 有天然优势（DM 奖励直接优化世界坐标位置），不宜直接比较。
> **Body_Pos_Err 和 Body_Rot_Err（root-relative）是更公平的对比维度。**

**关键发现**:
- DM 在局部姿态指标上优于 AMP **3.4-9.3 倍**（body_pos 9.3×，body_rot 6.5×，dof_vel 3.4×）
- AMP 的 Body_Pos_Err (0.079m) 和 Body_Rot_Err (0.287 rad) 说明局部姿态质量可接受
- DM 的 Body_Pos_Err 降到 0.0085m（约 8.5mm），几乎完美复现参考动作
- Root_Pos_Err 差距（18.9×）主要反映方法论差异而非质量差异——AMP 不跟踪全局位置是设计选择
...
|------|---------------------|----------------------|-------------|
| Root_Pos_Err (m) | **0.0375 ± 0.0000** | 0.4218 ± 0.0018 | 11.2× ★ |
| Root_Rot_Err (rad) | **0.0466 ± 0.0000** | 0.3674 ± 0.0004 | 7.9× |
| Body_Pos_Err (m) | **0.0246 ± 0.0000** | 0.1249 ± 0.0001 | **5.1×** |
| Body_Rot_Err (rad) | **0.1139 ± 0.0000** | 0.5163 ± 0.0006 | **4.5×** |
| Dof_Vel_Err (rad/s) | **1.0204 ± 0.0010** | 2.4947 ± 0.0006 | **2.4×** |
| Root_Vel_Err (m/s) | **0.1357 ± 0.0000** | 0.3753 ± 0.0001 | 2.8× ★ |
| Root_Ang_Vel_Err (rad/s) | **0.2612 ± 0.0002** | 1.0762 ± 0.0003 | 4.1× |

**关键发现**:
- spinkick 是更具动态性的动作（旋踢），两种方法的追踪误差都比 walk 高
- DM 在局部姿态上优于 AMP **2.4-5.1 倍**（body_pos 5.1×，body_rot 4.5×，dof_vel 2.4×）
- AMP spinkick 的 Root_Rot_Err = 0.367 rad（约 21°），说明 AMP 的朝向跟踪比训练日志中显示的好得多
...
|------|---------------|---------|
| **局部姿态精度** | body_pos 优 5-9×，body_rot 优 5-7× | — |
| **全局位置跟踪** | 精确跟踪参考轨迹（奖励直接优化） | 不跟踪（设计如此），无全局位移约束 |
| **鲁棒性** | 依赖精确参考，偏差过大会终止 | 无 pose termination，自由探索 |
| **局部姿态** | 接近完美（8.5mm body_pos_err） | 质量可接受（Body_Pos_Err < 0.08m for walk） |
...
|------|---------------------|----------------------|------|
| Test_Return | **279.85 ± 0.01** | 278.49 ± 0.01 | -0.5% |
| Root_Pos_Err | **0.0375 ± 0.0000** | 0.0395 ± 0.0001 | +5.3% |
| Root_Rot_Err | **0.0466 ± 0.0000** | 0.0539 ± 0.0000 | +15.7% |
| Body_Pos_Err | **0.0246 ± 0.0000** | 0.0258 ± 0.0000 | +4.9% |
| Body_Rot_Err | **0.1139 ± 0.0000** | 0.1170 ± 0.0000 | +2.7% |
| Dof_Vel_Err | **1.0204 ± 0.0010** | 1.0880 ± 0.0010 | +6.6% |
| Root_Vel_Err | **0.1357 ± 0.0000** | 0.1465 ± 0.0001 | +7.9% |
| Root_Ang_Vel_Err | **0.2612 ± 0.0002** | 0.2598 ± 0.0001 | -0.5% |
| Wall_Time (h) | 3.11 | 3.61 | +16% |

> 注：评估数据来自重评估（两个实验均统一 pose_termination=False），std 极小说明 3 seeds 间方差可忽略。
...
| 指标 | Exp-A (DM 无 term) | Exp4 (AMP) | DM 更优倍数 |
|------|---------------------|------------|-------------|
| Root_Pos_Err | **0.0395** | 0.4218 | 10.7× ★ |
| Body_Pos_Err | **0.0258** | 0.1249 | **4.8×** |
| Body_Rot_Err | **0.1170** | 0.5163 | **4.4×** |
| Dof_Vel_Err | **1.0880** | 2.4947 | **2.3×** |

即使移除 pose termination，DM 的局部姿态误差仍优于 AMP 2.3-4.8 倍。**DM 的精度优势来自显式追踪奖励设计，而非 pose termination 机制**。
...
|------|---------------------|----------------------|-------------|
| Root_Pos_Err | **0.0511 ± 0.0002** | 0.5371 ± 0.0033 | 10.5× ★ |
| Root_Rot_Err | **0.0700 ± 0.0002** | 0.1920 ± 0.0013 | 2.7× |
| Body_Pos_Err | **0.0258 ± 0.0000** | 0.0780 ± 0.0004 | **3.0×** |
| Body_Rot_Err | **0.1182 ± 0.0001** | 0.3217 ± 0.0019 | **2.7×** |
| Dof_Vel_Err | **0.8610 ± 0.0033** | 1.4156 ± 0.0102 | **1.6×** |
| Root_Vel_Err | **0.1376 ± 0.0000** | 0.2402 ± 0.0010 | 1.7× ★ |
| Root_Ang_Vel_Err | **0.2620 ± 0.0001** | 0.6879 ± 0.0034 | 2.6× |

#### 单技能 vs 多技能（DM 内部对比）

| 指标 | Exp1 (DM walk) | Exp2 (DM spinkick) | Exp5a (DM diverse) |
|------|----------------|---------------------|---------------------|
| Test_Return | **295.64** | 279.85 | 278.65 |
| Body_Pos_Err | **0.0085** | 0.0246 | 0.0258 |
| Body_Rot_Err | **0.0441** | 0.1139 | 0.1182 |
| Dof_Vel_Err | **0.2886** | 1.0204 | 0.8610 |
...
|------|----------------|---------------------|---------------------|
| Root_Pos_Err | 0.4497 | 0.4218 | 0.5371 |
| Body_Pos_Err | **0.0789** | 0.1249 | 0.0780 |
| Disc_Reward_Mean | 0.308 | **0.928** | 0.991 |

**发现**: AMP diverse 的 Body_Pos_Err (0.078m) 与 AMP walk (0.079m) 相当，优于 AMP spinkick (0.125m)。Disc_Reward_Mean (0.991) 比单动作实验都高，但这可能因为判别器在多动作数据下更容易被"骗"。
...
| 指标 | Exp5a (DM) | Exp5c (AMP) | Exp5b (ASE) | AMP/DM | ASE/DM |
|------|------------|-------------|-------------|--------|--------|
| Root_Pos_Err (m) | **0.0511** | 0.5371 | 4.3984 | 10.5× ★ | 86× ★ |
| Root_Rot_Err (rad) | **0.0700** | 0.1920 | 1.1135 | 2.7× | 15.9× |
| Body_Pos_Err (m) | **0.0258** | 0.0780 | 0.2668 | **3.0×** | **10.3×** |
| Body_Rot_Err (rad) | **0.1182** | 0.3217 | 1.3338 | **2.7×** | **11.3×** |
| Dof_Vel_Err (rad/s) | **0.8610** | 1.4156 | 2.0522 | **1.6×** | **2.4×** |
| Root_Vel_Err (m/s) | **0.1376** | 0.2402 | 0.5830 | 1.7× ★ | 4.2× ★ |
| Root_Ang_Vel_Err (rad/s) | **0.2620** | 0.6879 | 1.6795 | 2.6× | 6.4× |
| Test_Episode_Length | 299.9 | 300.0 | **299.3** | — | — |
...
|------|-------------|-----------------|-----------------|---------------|
| Test_Return | 295.64 | 279.85 | 278.65 | -5.7% (vs walk) |
| Body_Pos_Err | 0.0085 | 0.0246 | 0.0258 | +204% (vs walk) |
| Body_Rot_Err | 0.0441 | 0.1139 | 0.1182 | +168% (vs walk) |
...
|------|-------------|-----------------|-----------------|
| Body_Pos_Err | **0.0789** | 0.1249 | 0.0780 |
| Body_Rot_Err | **0.2871** | 0.5163 | 0.3217 |
| Disc_Reward_Mean | 0.308 | 0.928 | **0.991** |

### 5.4 Layer 3 深度分析

#### 发现 1: DM 在局部姿态上仍然全面领先

DM diverse 的 body_pos_err (0.026m) 比 AMP (0.078m) 低 3×，比 ASE (0.267m) 低 10×。这是方法论差异：DM 直接优化追踪，AMP/ASE 优化分布匹配。

#### 发现 2: ASE 的追踪误差反而高于 AMP

| 指标 | AMP diverse | ASE diverse | ASE vs AMP |
|------|-------------|-------------|------------|
| Body_Pos_Err | **0.0780** | 0.2668 | 3.4× worse |
| Body_Rot_Err | **0.3217** | 1.3338 | 4.1× worse |
| Dof_Vel_Err | **1.4156** | 2.0522 | 1.5× worse |

ASE 在所有局部姿态指标上**比 AMP 差 1.5-4.1 倍**。这可能因为：
...
|------|------------|-------------|-------------|
| **局部姿态精度** | **最优** (Body_Pos 0.026m) | 中等 (0.078m) | 最差 (0.267m) |
| **稳定性** | 好 (ep_len 299.9) | 好 (300.0) | 好 (299.3) |
...
|------|-------------------|-----------------|------|
| Root_Pos_Err | 7.2795 | 0.4497 | 16× worse ★ |
| Body_Pos_Err | 0.2292 | **0.0789** | 2.9× worse |
| Body_Rot_Err | 1.5092 | **0.2871** | 5.3× worse |
| Dof_Vel_Err | 1.3826 | **0.9951** | 1.4× worse |

Exp6 的局部姿态误差高于纯 AMP walk 1.4-5.3×，这符合预期——steering 要求 agent 跟随随机方向/速度指令，无法完全复现参考运动。Root_Pos_Err 极大（7.28m）是因为 steering 任务要求 agent 主动移动到不同位置。
...
| 研究问题 | 结论 |
|----------|------|
| **DM vs AMP: 精度 vs 鲁棒性?** | DM 局部姿态精度优 2.4-9.3×（body_pos/rot_err），root_pos_err 差距更大但不公平（DM 直接优化）。两者回合存活率相当（~100%） |
| **Pose termination 多关键?** | 影响收敛速度（前 1000 迭代差 61%），但不影响最终性能（差 <1%）。非防止策略崩溃的必要条件 |
| **DM vs AMP vs ASE 多技能?** | DM 追踪最优；ASE 局部姿态反而比 AMP 差 1.5-4.1×（优化目标分散 + 训练不足）；ASE Diversity 未收敛（需 >500M 样本） |
| **AMP 先验迁移?** | 成功迁移到 steering 任务，但存在 task-style trade-off（局部姿态误差增大 1.4-5.3×） |

### 7.2 视觉观察总结

全部 9 个实验已录制视频（`output/videos/`），详细说明见 `output/videos/README.md`。

#### 方法论差异（代码分析 + 视觉验证）

| 维度 | DeepMimic | AMP | ASE |
|------|-----------|-----|-----|
| 本质 | 被动跟踪器：phase 取模循环播放参考轨迹，agent 逐帧跟踪 | 主动生成器：每步根据当前身体状态自主决策，判别器给奖励 | 条件生成器：根据 (状态, latent z) 生成动作 |
| 动作循环 | 强制循环（参考轨迹 phase 取模，agent 无选择权） | 涌现循环（策略学到"踢完再踢"奖励最高） | 由 z 控制（理论上不同 z 不同动作） |
| Episode 内切换 | 不能（motion_id 在 reset 时固定） | 不能（单高斯策略，无动作选择输入） | 理论上能（换 z），实际 Diversity 未收敛 |
| 外部命令 | 不能（观测空间无命令输入） | 能（如 Exp6 steering：方向/速度命令作为观测输入） | 能（z 作为技能选择器） |

#### 动作片段特性

| 动作 | 时长 | Loop | 首尾帧连续性 | 每周期位移 |
|------|------|------|------------|-----------|
| walk | 1.03s | WRAP | 完美（关节/朝向 diff=0） | X+1.3m（匀速前进） |
| spinkick | 1.28s | WRAP | 不完美（朝向差 14.7°，关节差 49.7°） | X-0.4m, Y+0.28m |
| dance_a | 1.62s | WRAP | 不完美（朝向差，侧向移动） | Y+1.15m |

#### Exp6 Steering 指令机制

策略观测中包含 `[local_tar_dir(2), tar_speed(1), local_face_dir(2)]`。
Episode 内每 4-7 秒随机切换一次新指令（方向 [-180°, 180°]，速度 [0.5, 5.0] m/s）。
没有专门的过渡奖励——策略在训练中经历大量指令切换，自然学会平滑过渡。

### 7.3 方法选型建议

| 场景 | 推荐方法 | 理由 |
|------|---------|------|
| 精确复现单个动作片段 | DeepMimic | 局部姿态精度优 2.4-9.3×（body_pos_err） |
| 学习运动风格 + 组合任务 | AMP | 判别器奖励可与任意任务奖励叠加，局部姿态质量可接受（<0.08m） |
| 多技能覆盖 | ASE（需充足训练） | latent 编码器可区分技能，但 500M 样本下 Diversity 未收敛 |

### 7.3 局限性

1. 训练为单种子 (seed=42)；评估已改为 3 seeds × 4096 episodes（std 极小，评估方差可忽略）
2. 未进行地形/障碍实验
3. root_pos_err 对 DM 有天然优势（DM 奖励直接在世界坐标系优化全局位置），本报告以局部姿态指标 (body_pos/rot_err) 为主要对比维度
4. Exp5a vs Exp5c 是方法级对比，非单变量消融
5. ASE 在 500M 样本下未完全收敛（Diversity_Loss = 0.96），结论可能因训练不足而偏保守

### 7.4 待补充分析

- [x] ~~追踪误差重评估~~：已完成（3 seeds × 4096 episodes，统一 pose_termination=False）
- [x] ~~视觉观察~~：已录制全部 9 个实验视频，确认 DM 精确跟踪 vs AMP 风格近似的差异
- [x] ~~Exp6 视频~~：3 episodes，每 episode 内指令每 4-7s 切换
